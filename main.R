...
# Function to clean data - simplified
clean_data <- function(data) {
  ...
  # Clean text data
  if ("Product_Collection" %in% names(data)) {
    data <- data %>%
      mutate(Product_Collection = clean_text_data(Product_Collection))
  }
  ...
  data <- data %>%
    mutate(
      ...
      Launch_Year = str_extract(Launch_Date, "\\d{2}$"),
      Launch_Year = ifelse(!is.na(Launch_Year), 
                           paste0("20", Launch_Year),
                           NA),
      
      # Process Bus_Speed with split value and unit
      ...
      # Other numeric processing
      Cache = sapply(Cache, get_num),
  ...
  # Remove original column if split
  if ("Bus_Speed" %in% names(data)) {
    data <- data %>% select(-Bus_Speed)
  }
...
impute_missing <- function(data) {
  ...
  # For character columns, use mode imputation
